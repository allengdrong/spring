...
//		+ vo.getMultipartFile().getOriginalFilename());
	...
	public String updateFile(ImageVO vo, PageObject pageObject, String deleteFile, HttpServletRequest request)
			throws Exception {
	...
	public String update(ImageVO vo, PageObject pageObject) throws Exception {
		...
		// DB에 이미지 정보 수정(제목, 내용) - 글번호에 맞는 정보.
		service.update(vo);
	...
	public String delete(Long no, String deleteFile, Long perPageNum, 
			HttpServletRequest request) throws Exception {

		log.info("delete() : no = " + no + ", deleteFile = " + deleteFile + ", perPageNum = " + perPageNum);

		// DB에서 파일 지우기 : Controller - Service - Mapper
		int result = service.delete(no);
		if (result == 0)
			throw new Exception("이미지 게시판 삭제 실패");

		// DB에서 삭제가 완료되었다.
		// 서버에서 파일 지우기 - 실제적인 위치의 파일이 필요하다.
		// getRealPath() - FileUtil.getRealPath() 을 import static으로 선언해서 메서드만 사용 가능
		//if (!FileUtil.delete(getRealPath(request, deleteFile)))
			throw new Exception("이미지 게시판 이미지 삭제 실패");
