...
public class TqController {
	...
	public String write(TqVO vo, int perPageNum, RedirectAttributes rttr,Long no) throws Exception {
		...
		return "redirect:write02.do?no=" + service.getMax(no) + "&page=1&perPageNum=" + perPageNum;
	}

	// 3-2. 문제 출제 게시판 등록 폼 write02 - get
	@GetMapping("/write02.do")
	// Model 객체 - 처리된 데이터를 JSP에 전달
	public String write02(Model model, Long no, @ModelAttribute PageObject pageObject) throws Exception {

		log.info("write02().no : " + no + " - 보기 글쓰기 --------------");

		model.addAttribute("vo", service.write02(no));
